...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #darksilkTEST\r");
            Send(hSocket, "WHO #darksilkTEST\r");
        } else
        {
	    LogPrintf("JOIN #darksilk\n");
            // randomly join #darksilk00-#darksilk05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #darksilk%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #darksilk%02d\r", channel_number).c_str());
        };
