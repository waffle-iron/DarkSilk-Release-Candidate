...
std::vector<CBudgetProposal*> CBudgetManager::GetBudget()
{
    ...
    while(it2 != vBudgetPorposalsSort.end())
    {
        ...
        if(pbudgetProposal->fValid && pbudgetProposal->nBlockStart <= nBlockStart &&
                ...
                pbudgetProposal->GetYeas() - pbudgetProposal->GetNays() > snodeman.CountEnabled(MIN_BUDGET_PEER_PROTO_VERSION)/10 &&
...
{
    if(GetNays() - GetYeas() > snodeman.CountEnabled(MIN_BUDGET_PEER_PROTO_VERSION)/10){
    ...
    */
    ...
    //     if(GetYeas()-GetNays() < (snodeman.CountEnabled(MIN_BUDGET_PEER_PROTO_VERSION)/10)) {
