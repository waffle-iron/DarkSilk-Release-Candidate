...
extern string getCurrencyToDRKSLKFromAlias(const vector<unsigned char> &vchCurrency, int64 &nFee, const unsigned int &nHeightToFind, vector<string>& rateList, int &precision);
int64 convertCurrencyCodeToDarksilk(const vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision)
{
	double drkslkPrice = nPrice;
	int64 nRate;
	vector<string> rateList;
	if(getCurrencyToDRKSLKFromAlias(vchCurrencyCode, nRate, nHeight, rateList, precision) == "")
	{
		// nRate is assumed to be rate of USD/DRKSLK
		drkslkPrice = drkslkPrice * nRate;
	}
	return (int64)drkslkPrice;
}
...
string makeOfferLinkAcceptTX(COfferAccept& theOfferAccept, const vector<unsigned char> &vchPubKey, const vector<unsigned char> &vchOffer, const vector<unsigned char> &vchOfferAcceptLink)
{
	...
	CDarkSilkAddress refundAddr = CDarkSilkAddress(newDefaultKey.GetID());
...
string makeOfferRefundTX(const CTransaction& prevTx, COffer& theOffer, const COfferAccept& theOfferAccept, const vector<unsigned char> &refundCode)
{
	...
	if(refundCode == OFFER_REFUND_COMPLETE)
	{
		...
		// lookup the price of the offer in darksilk based on pegged alias at the block # when accept was made (sets nHeight in offeraccept serialized object in tx)
		nTotalValue = convertCurrencyCodeToDarksilk(theOffer.sCurrencyCode, theOffer.GetPrice(entry), theOfferAccept.nHeight, precision)*theOfferAccept.nQty;
	...
	if(refundCode == OFFER_REFUND_COMPLETE)
	{
		...
		CDarkSilkAddress refundAddress(stringFromVch(theOfferAccept.vchRefundAddress));
...
bool GetNameOfOfferTx(const CTransaction& tx, vector<unsigned char>& offer) {
	...
	if (!DecodeOfferTx(tx, op, nOut, vvchArgs, -1))
		return error("GetNameOfOfferTx() : could not decode a darksilk tx");
...
bool CheckOfferInputs(CBlockIndex *pindexBlock, const CTransaction &tx,
	...
	if (!tx.IsCoinBase()) {
		...
		if (tx.nVersion != DRKSLK_TX_VERSION) {
			if (found)
				return error(
						"CheckOfferInputs() : a non-darksilk transaction with a darksilk input");
			return true;
		}
		...
		if (!good)
			return error("CheckOfferInputs() : could not decode a darksilk tx");
		int nDepth;
		int64 nNetFee;
			...
			if (!fMiner && !fJustCheck && pindexBlock->nHeight != pindexBest->nHeight) {
				...
				else if(op == OP_OFFER_REFUND)
				{
					...
					{	
						...
						int precision = 2;
						// lookup the price of the offer in darksilk based on pegged alias at the block # when accept was made (sets nHeight in offeraccept serialized object in tx)
						int64 nPrice = convertCurrencyCodeToDarksilk(theOffer.sCurrencyCode, theOffer.GetPrice(entry), theOfferAccept.nHeight, precision)*theOfferAccept.nQty;
						for(unsigned int i=0;i<tx.vout.size();i++)
						{
...
Value offernew(const Array& params, bool fHelp) {
	...
	vector<string> rateList;
	int precision;
	if(getCurrencyToDRKSLKFromAlias(vchCurrency, nRate, nBestHeight, rateList,precision) != "")
	{
		throw JSONRPCError(RPC_INVALID_PARAMETER, "Could not find this currency code in the DRKSLK_RATES alias!\n");
	}
...
Value offerlink(const Array& params, bool fHelp) {
	...
	int precision = 2;
	// get precision
	convertCurrencyCodeToDarksilk(linkOffer.sCurrencyCode, linkOffer.GetPrice(), nBestHeight, precision);
	float minPrice = 1/pow(10,precision);
...
Value offerupdate(const Array& params, bool fHelp) {
	...
	int precision = 2;
	// get precision
	convertCurrencyCodeToDarksilk(theOffer.sCurrencyCode, theOffer.GetPrice(), nBestHeight, precision);
	float minPrice = 1/pow(10,precision);
	if(price < minPrice)
		price = minPrice;
	string priceStr = strprintf("%.*f", precision, price);
...
Value offeraccept(const Array& params, bool fHelp) {
	...
	if(params.size() < 5)
	{
		CPubKey newDefaultKey;
		pwalletMain->GetKeyFromPool(newDefaultKey, false);
		refundAddr = CDarkSilkAddress(newDefaultKey.GetID());
		vchRefundAddress = vchFromString(refundAddr.ToString());
	}
	else
	{
		vchRefundAddress = vchFromValue(params[4]);
		refundAddr = CDarkSilkAddress(stringFromVch(vchRefundAddress));
	}
	...
	int precision = 2;
	int64 nPrice = convertCurrencyCodeToDarksilk(theOffer.sCurrencyCode, theOffer.GetPrice(foundCert), nBestHeight, precision);
	...
	if(!foundCert.IsNull())
	{
		...
		CWalletTx wtxCert;
		wtxCert.nVersion = DRKSLK_TX_VERSION;
		CScript scriptPubKeyOrig;
		CCert cert;
		CTransaction tx;
...
Value offerinfo(const Array& params, bool fHelp) {
	...
	string offer = stringFromVch(vchOffer);
	{
		...
		for(unsigned int i=0;i<theOffer.accepts.size();i++) {
			...
			int precision = 2;
			convertCurrencyCodeToDarksilk(theOffer.sCurrencyCode, 0, nBestHeight, precision);
...
Value offerlist(const Array& params, bool fHelp) {
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet)
        {
			...
			int precision = 2;
			convertCurrencyCodeToDarksilk(theOfferA.sCurrencyCode, 0, nBestHeight, precision);
...
Value offerfilter(const Array& params, bool fHelp) {
	...
	BOOST_FOREACH(pairScan, offerScan) {
		...
		int precision = 2;
		convertCurrencyCodeToDarksilk(txOffer.sCurrencyCode, 0, nBestHeight, precision);
