// Copyright (c) 2015 The Crave Developer (niitassin)
...
void MarketInit()
{
    ...
    if(TryOpenMarketDB())
    {
	LogPrintf("Opened local market database.\n");
	LOCK(cs_markets);
	...
	BOOST_FOREACH(PAIRTYPE(const uint256, CSignedMarketListing)& p, mapListings)
	{
		...
		mapListingsByCategory.insert(make_pair(p.second.listing.sCategory, setListings));
	    }
	    else
...
bool ReceivePaymentRequest(CPaymentRequest request)
{
    ...
    {
	mapPaymentRequests.insert(make_pair(request.GetHash(), request));
...
bool ReceiveEscrowPayment(CEscrowPayment escrow)
{
    ...
    {
	mapEscrowPayments.insert(make_pair(escrow.GetHash(), escrow));
...
bool ReceiveEscrowRelease(CEscrowRelease escrow)
{
    ...
    {
	mapEscrowReleases.insert(make_pair(escrow.GetHash(), escrow));
...
bool ReceiveBuyAccept(CBuyAccept accept)
{
    ...
    {
	mapBuyAccepts.insert(make_pair(accept.GetHash(), accept));
...
bool ReceiveBuyReject(CBuyReject reject)
{
    ...
    {
	mapBuyRejects.insert(make_pair(reject.GetHash(), reject));
...
bool ReceiveDeliveryDetails(CDeliveryDetails details)
{
    ...
    {
	mapDeliveryDetails.insert(make_pair(details.GetHash(), details));
...
bool ReceiveRefundRequest(CRefundRequest refund)
{
    ...
    {
	mapRefundRequests.insert(make_pair(refund.GetHash(), refund));
...
void ReceiveCancelListing(CCancelListing cancel)
{
    ...
    mapCancelListings.insert(make_pair(cancel.GetHash(), cancel));
...
bool ReceiveBuyRequest(CBuyRequest buyr)
{
    ...
    {
        mapBuyRequests.insert(make_pair(buyr.requestId, buyr));
        WriteBuyRequests(mapBuyRequests);
    }
...
void ReceiveListing(CSignedMarketListing listing)
{
	if(mapListings.find(listing.GetHash()) == mapListings.end())
	    ...
	    mapListings.insert(make_pair(listing.GetHash(), listing));
	    WriteListings(mapListings);
	    // Update category
		...
		mapListingsByCategory.insert(make_pair(listing.listing.sCategory, setListings));
...
bool MarketDB::Open(const char* pszMode)
{
    ...
    {
        LogPrintf("MarketDB::open() - DB does not exist.");
        return false;
    };
    ...
    if (!s.ok())
    {
        LogPrintf("MarketDB::open() - Error opening db: %s.", s.ToString().c_str());
        return false;
    };
...
bool MarketDB::ScanBatch(const CDataStream& key, std::string* value, bool* deleted) const
{
    ...
    *deleted = false;
    ...
    if (!s.ok())
    {
        LogPrintf("MarketDB ScanBatch error: %s", s.ToString().c_str());
        return false;
    };
...
bool MarketDB::TxnCommit()
{
    ...
    if (!status.ok())
    {
        LogPrintf("MarketDB batch commit failure: %s", status.ToString().c_str());
        return false;
    };
