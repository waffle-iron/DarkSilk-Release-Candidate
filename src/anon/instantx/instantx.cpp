...
//step 2.) Top INSTANTX_SIGNATURES_TOTAL stormnodes, open connect to top 1 stormnode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 stormnode, waits for INSTANTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
            ...
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                ...
                if(mapUnknownVotes[ctx.vinStormnode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinStormnode.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::ix - stormnode is spamming transaction votes: %s %s\n",
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    BOOST_REVERSE_FOREACH(CTxIn i, tx.vin){
        ...
        if(nTxAge < 9) //9 confirmations, 1 less than the "send IX" gui requires, incase of a block propagating the network at the time
        {
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())){
        ...
        newLock.nExpiration = GetTime()+(60*60); //locks expire after 60 minutes (56-60 confirmations)
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown Stormnode\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Stormnode not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if(psn != NULL)
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Stormnode ADDR %s %d\n", psn->addr.ToString().c_str(), n);

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Unknown Stormnode\n");
        snodeman.AskForSN(pnode, ctx.vinStormnode);
        return false;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Stormnode not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }
...
bool CConsensusVote::Sign()
{
    ...
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid stormnode privkey: '%s'\n", errorMessage.c_str());
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    ...
    BOOST_FOREACH(CConsensusVote vote, vecConsensusVotes)
    {
        ...
        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Stormnode\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Stormnode not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
