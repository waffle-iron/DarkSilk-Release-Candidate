...
bool GetNameOfCertTx(const CTransaction& tx, vector<unsigned char>& cert) {
    ...
    if (!DecodeCertTx(tx, op, nOut, vvchArgs, -1))
        return error("GetNameOfCertTx() : could not decode a darksilk tx");
...
bool CheckCertInputs(CBlockIndex *pindexBlock, const CTransaction &tx,
    ...
    if (!tx.IsCoinBase()) {
        ...
        if (tx.nVersion != DRKSLK_TX_VERSION) {
            if (found)
                return error(
                        "CheckCertInputs() : a non-darksilk transaction with a darksilk input");
            return true;
        }
        vector<vector<unsigned char> > vvchArgs;
        int op, nOut;
        ...
        if (!good)
            return error("CheckCertInputs() : could not decode a darksilk tx");
        int nDepth;
        int64 nNetFee;
        ...
        case OP_CERT_ACTIVATE:
            if (found)
                return error(
                        "CheckCertInputs() : certactivate tx pointing to previous darksilk tx");
 ...
 if (fHelp || params.size() < 2 || params.size() > 3)
    ...
    if(theCert.bPrivate)
    {
        try
        {
            ...
            CPubKey PubKey(vchPubKeyByte);
            CKeyID pubKeyID = PubKey.GetID();
            sendAddr = CDarkSilkAddress(pubKeyID);
        }
        catch(std::exception &e)
        {
    ...
    {
        sendAddr = CDarkSilkAddress(stringFromVch(vchPubKeyOrAddress));
    }
