...
{
   CAmount threshold;
   LargerOrEqualThanThreshold(CAmount threshold) : threshold(threshold) {}
   bool operator()(pair<pair<CAmount,CAmount>,pair<const CWalletTx*,unsigned int> > const &v) const { return v.first.first >= threshold; }
};
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CDarkSilkAddress(redeemScript.GetID()).ToString();
...
void CWalletTx::GetAmounts(list<pair<CTxDestination, CAmount> >& listReceived,
                           list<pair<CTxDestination, CAmount> >& listSent, CAmount& nFee, string& strSentAccount, const isminefilter& filter) const
{
    nFee = 0;
    listReceived.clear();
    listSent.clear();
                                  ...
                                  CAmount& nSent, CAmount& nFee, const isminefilter& filter) const
{
    ...
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, CAmount)& s, listSent)
            nSent += s.second;
        nFee = allFee;
    }
    {
        LOCK(pwallet->cs_wallet);
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, CAmount)& r, listReceived)
        {
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s DRKSLK %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
int CWallet::GetRealInputSandstormRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("sandstorm", "GetInputSandstormRounds INSERTING %s\n", hash.ToString());
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputSandstormRounds(in2, rounds+1);
...
int CWallet::GetInputSandstormRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realSandstormRounds = GetRealInputSandstormRounds(in, 0);
    return realSandstormRounds > nSandstormRounds ? nSandstormRounds : realSandstormRounds;
}
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                ...
                if (pcoin->vout[i].nValue == 10000*COIN){
                    ...
                    //LogPrintf("CWallet::AvailableCoinsForStaking - Found Stormnode collateral.\n");
                    found = true;
                    break;
                }
...
static void ApproximateBestSubset(vector<pair<CAmount, pair<const CWalletTx*,unsigned int> > >vValue, CAmount nTotalLower, CAmount nTargetValue,
...
bool CWallet::SelectCoinsMinConfByCoinAge(const CAmount& nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    CAmount nGCD = vValue[0].first.first;
    ...
    CAmount denom = nGCD;
    const CAmount k = 25;
    const CAmount approx = CAmount(vValue.size() * (nTotalValue - nTargetValue)) / k;
    if (approx > nGCD)
    {
    ...
    CAmount costsum = 0;
    vector<vector<pair<pair<CAmount,CAmount>,pair<const CWalletTx*,unsigned int> > >::iterator> vZeroValueBundles;
    if (denom != nGCD)
    {
        ...
        vector<pair<pair<CAmount,CAmount>,pair<const CWalletTx*,unsigned int> > >::iterator itZeroValue = std::stable_partition(vValue.begin(), vValue.end(), LargerOrEqualThanThreshold(denom));
        vZeroValueBundles.push_back(itZeroValue);
        pair<CAmount, CAmount> pBundle = make_pair(0, 0);
        ...
        if (nTargetValue/denom >= nTotalValue)
        {
            ...
            for (; nBeginCoinValues < nTotalCoinValues && (nTargetValue - nValueRet)/denom >= nTotalValue; ++nBeginCoinValues)
            {
                if (nBeginCoinValues >= nBeginBundles)
                {
                    ...
                    for (vector<pair<pair<CAmount,CAmount>,pair<const CWalletTx*,unsigned int> > >::iterator it = vZeroValueBundles[nBundle]; it != vZeroValueBundles[nBundle + 1]; ++it)
                    {
    ...
    if (nValueRet < nTargetValue && !vZeroValueBundles.empty())
    {
        ...
        for (vector<pair<pair<CAmount,CAmount>,pair<const CWalletTx*,unsigned int> > >::iterator it = vZeroValueBundles.back(); it != vValue.end() && nValueRet < nTargetValue; ++it)
        {
             setCoinsRet.insert(it->second);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
        ...
        BOOST_FOREACH(CAmount v, sandStormDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1DRKSLK over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputSandstormRounds(vin);
                    // make sure it's actually anonymized
    ...
    boost::function<bool (const CWallet*, const CAmount&, unsigned int, int, int, std::vector<COutput>, std::set<std::pair<const CWalletTx*,unsigned int> >&, CAmount&)> f;
    // f uses SelectCoinsMinConfByCoinAge for the first 250 blocks then without it.
    if (pindexBest->nHeight <= 250)
        f = fMinimizeCoinAge ? &CWallet::SelectCoinsMinConfByCoinAge : &CWallet::SelectCoinsMinConf;
    else
        f = &CWallet::SelectCoinsMinConf;
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nSandstormRoundsMin, int nSandstormRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputSandstormRounds(vin);
            if(rounds >= nSandstormRoundsMax) continue;
            if(rounds < nSandstormRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nSandstormRoundsMin, int nSandstormRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nSandstormRoundsMin < 0 ? ONLY_NONDENOMINATED_NOTSN : ONLY_DENOMINATED);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: Sandstorm requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    CCoinControl *coinControl=NULL;
    int nChangePos = -1;
    //bool success = CreateTransaction(vecSend, tx, keyChange, nFeeRequired, nChangePos, strFailReason, coinControl);
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, nChangePos, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if(!success){
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int32_t& nChangePos, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough Sandstorm non-denominated funds for this transaction.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOTSN) {
                        strFailReason = _("Unable to locate enough Sandstorm non-denominated funds for this transaction that are not equal 1000 DRKSLK.");
                    } else {
                        strFailReason = _("Unable to locate enough Sandstorm denominated funds for this transaction.");
                        strFailReason += _("Sandstorm uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                {
                    ...
                    //But mempool inputs might still be in the mempool, so their age stays 0                    int age = pcoin.first->GetDepthInMainChain();
                ...
                {
                    CAmount nMoveToFee = min(nChange, nFee - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }
...
bool CWallet::CreateTransaction(CScript scriptPubKey, CAmount nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
    vector< pair<CScript, CAmount> > vecSend;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CDarkSilkAddress coinAddress(keyID);
...
bool CWallet::CreateStealthTransaction(CScript scriptPubKey, CAmount nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckKernel(pindexPrev, nBits, txNew.nTime - n, prevoutStake, &nBlockTime))
            {
                ...
                if (nCredit >= GetStakeSplitThreshold()) //TODO (AA): Remove this.  Test if affects PoS
...
string CWallet::PrepareSandstormDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        else if((sandStormPool.sessionDenom & (1 << 1)) && v == ((1000*COIN)  +1000000)) {fAccepted = true;}
        else if((sandStormPool.sessionDenom & (1 << 2)) && v == ((100*COIN)   +100000)) {fAccepted = true;}
        else if((sandStormPool.sessionDenom & (1 << 3)) && v == ((10*COIN)    +10000)) {fAccepted = true;}
        else if((sandStormPool.sessionDenom & (1 << 4)) && v == ((1*COIN)     +1000)) {fAccepted = true;}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CDarkSilkAddress(address).ToString());
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                CAmount n = pcoin->IsSpent(i) ? 0 : pcoin->vout[i].nValue;
...
void CWallet::FixSpentCoins(int& nMismatchFound, CAmount& nBalanceInQuestion, bool fCheckOnly)
{
    nMismatchFound = 0;
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s DRKSLK %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s DRKSLK %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
