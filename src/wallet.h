...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTSN = 4 // ONLY_NONDENOMINATED and not 1000 DRK at the same time
};
...
public:
    ...
    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, vector<COutput>& vCoins, int64_t& nValueRet, int nSandstormRoundsMin, int nSandstormRoundsMax);
...
public:
    ...
    //Used with Sandstorm. Will return fees, then denominations, everything else, then very small inputs that aren't fees
    int Priority() const
    {
        if(tx->vout[i].nValue == SANDSTORM_FEE) return -20000;
