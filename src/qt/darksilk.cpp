...
static void InitMessage(const std::string &message)
{
    if(splashref)
    {
        splashref->showMessage(QString::fromStdString(message), Qt::AlignBottom|Qt::AlignHCenter, QColor(67,67,67));
...
int main(int argc, char *argv[])
{
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        ...
        // (which not yet possible because lang=XX can be overridden in darksilk.conf in the data directory)
        QMessageBox::critical(0, "DarkSilk",
    ...
    if(GetBoolArg("-testnet", false)) // Separate UI settings for testnet
        app.setApplicationName("DarkSilk-Qt-testnet");
    else
        app.setApplicationName("DarkSilk-Qt");
