...
void DarkSilkAmountField::setText(const QString &text)
{
    if (text.isEmpty())
        amount->clear();
    else
...
bool DarkSilkAmountField::validate()
{
    ...
    if (amount->value() == 0.0)
        valid = false;
    if (valid && !DarkSilkUnits::parse(currentUnit, text(), 0))
        valid = false;
...
QString DarkSilkAmountField::text() const
{
...
qint64 DarkSilkAmountField::value(bool *valid_out) const
{
    qint64 val_out = 0;
...
void DarkSilkAmountField::setValue(qint64 value)
{
    setText(DarkSilkUnits::format(currentUnit, value));
}
...
void DarkSilkAmountField::unitChanged(int idx)
{
    ...
    amount->setDecimals(DarkSilkUnits::decimals(currentUnit));
    amount->setMaximum(qPow(10, DarkSilkUnits::amountDigits(currentUnit)) - qPow(10, -amount->decimals()));
